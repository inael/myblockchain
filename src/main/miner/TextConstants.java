public class TextConstants {
    public static final String MINER_ADDING_NEW_BLOCK_S = "Miner '%s' adding new Block '%s'";
    public static final String ENTER_THE_NUMBER_OF_THREADS = "Enter the number of Threads";
    public static final String ENTER_THE_NUMBER_OF_MINERS = "Enter the number of miners: ";
    public static final String ENTER_THE_NUMBER_OF_BLOCKS_PER_MINER = "Enter the number of blocks per miner:";
    public static final String ENTER_A_NUMBER_TO_REPRESENT_THE_MINING_DIFFICULTY_EG_1_1000 = "Enter a number to " +
            "represent the mining difficulty (eg 1...1000) ";
    public static final String BLOCKCHAIN_IS_VALID = "Blockchain is Valid %s: ";
    public static final String THE_BLOCK_CHAIN = "The block chain: \n%s";
    public static final String TIME_ELAPSED_TO_END_MINERS = "Time elapsed to end miners: %s";

}
